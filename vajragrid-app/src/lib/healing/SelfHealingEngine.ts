...
/** Process new alerts — triggers healing for CRITICAL/HIGH threats */
export function processAlerts(alerts: ThreatAlert[]) {
  ...
  for (const alert of alerts) {
    ...
    for (const busId of alert.affectedAssets) {
      // Don't start duplicate healing for same bus
      ...
      addAction(event, 'DETECTING', 'Threat detected', busId,
        `${alert.severity} ${alert.threatCategory} alert on ${busId}. Initiating VajraShield response.`);
